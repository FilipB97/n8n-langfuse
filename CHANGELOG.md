...
- **Reworked the demo workflow into grouped lanes.** `docs/example-workflow.json`
  no longer runs as one long sequential chain; it now fans out from
  `Manual → Setup → OpenAI` into four parallel, colored lanes — ingestion, Public
  API reads, datasets, and prompts — so the whole demo is readable at a glance.
  It also now covers the dataset and Create Prompt operations. Generated by
  `scripts/gen-example-workflow.mjs`.
