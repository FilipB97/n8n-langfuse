...
- On `Span`/`Generation`/`Event`/`Score` operations the `Trace ID` **auto-fills from the previous step's output in code** (the node reads `traceId` off the incoming item), so chaining `Trace Create → Span Create` links them with no manual wiring. If the span runs **several steps later** (not directly after Trace Create), reveal Advanced Fields and set `Trace ID` by name instead, e.g. `={{ $('Trace Create').item.json.traceId }}`
- Minimal by default: only the fields you actually type are visible — content (`Name`, `Input`/`Output JSON`, `Model`, the generation's `Prompt`/`Completion`/`Total Tokens`) and the required score/SDK/batch fields. The plumbing the node fills itself — `Trace ID`, `Observation ID`, `Parent Observation ID`, `Session ID`, `User ID`, generation `Prompt Name`/`Prompt Version` — plus genuinely optional fields (metadata, tags, version, level, status message, timing, model parameters, raw usage/cost JSON) live under **Show Advanced Fields**. Turn it on to set any of them explicitly
- `Span Create`, `Generation Create`, `Event Create`, and `SDK Log Create` auto-generate observation ids when needed
- `Span Update`, `Generation Update`, and `Finalize Span` reference an existing `Observation ID`, auto-filled in code from the previous step's `observationId` (reveal Advanced Fields to set it explicitly); `Finalize Span` exposes the full generation field set — model, input/output, usage, etc.
- `Score Create` requires a score value plus either a trace id or a session id
- `Span`/`Generation`/`Event Create` always pin a `traceId` — minting one when there is nothing to inherit — and report it on the output, so every observation in a chain lands in the same trace. Updates never invent one (that would re-point the existing observation)
- `Finalize Span` sends its `generation-create` and `span-update` against the same trace, nests the generation under the span, and closes both at `endTime`
- `timestamp` is generated automatically when missing, and is normalized to ISO 8601 with milliseconds — `Date` values, epoch seconds/milliseconds, and loose strings like `2026-06-02 10:00:00` are all accepted
- timing is auto-filled: `Span`/`Generation Create` default `startTime`, updates default `endTime`, and `Finalize Span` closes the span — so observations always render with timing in Langfuse. An explicit `Timestamp` supplies those defaults; the timing fields override it
- token usage and cost are keyed by usage type (`input`, `output`, `total`) — set `Prompt`/`Completion`/`Total Tokens`, or pass an LLM node's usage object as `Usage Details JSON`: the common spellings (`prompt_tokens`, `completionTokens`, `total_cost`, …) are mapped onto those buckets and string values are coerced to numbers, which Langfuse requires
